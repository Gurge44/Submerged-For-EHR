...
public sealed class ReconnectPipingMinigame(nint ptr) : Minigame(ptr)
{
    public int[] seeds =
    [
        326907259, 407357077, 2001864847, 20885327, 2118582405, 1545346156, 276236684, 436702103, 1613700476,
        1151508687, 1908408660, 782524822, 38597829, 1823140859, 285183977, 116038448, 963051378, 2133468912, 60144147,
        2081408537, 127617096, 1520984973, 64894018, 737729816, 1490420376, 1964503931, 663335080, 1055545808,
        2048850280, 1834039874, 425877024, 1020089911, 203988300, 904052500, 2097071838, 1815037601, 44206942,
        418787165, 2037043467, 402392094, 1095217546, 1769336404, 1911757930, 574371127, 1095048023, 563977301,
        1943992207, 1057236064, 1819439226, 2104842910, 568151366, 509610580, 1816391224, 615074661, 845740689,
        600989049, 262237723, 33807084, 201414864, 625200086, 2033357123, 770133455, 885000920, 2030125499, 1095048023,
        563977301, 1943992207, 1057236064, 1819439226, 2104842910, 568151366, 509610580, 1816391224, 615074661,
        845740689, 600989049, 262237723, 33807084, 201414864, 625200086, 2033357123, 770133455, 885000920, 2030125499
    ];

    private Cell[,] _cells;
    private SCG.List<(int xPos, int yPos)> _path;
    ...
    private void Start()
    {
        ...
        SCG.Dictionary<int, int> dict = new();
    ...
    {
        SCG.List<(int xPos, int yPos)> visitedPositions = [];
    ...
    public void AssignDefaultCells()
    {
        ...
        for (int x = 0; x < 4; x++)
        {
            for (int y = 0; y < 6; y++)
            {
                cells[x, y] = new()
                {
                    leftWall = x == 0 ? new() : cells[x - 1, y].rightWall,
                    rightWall = new(),
                    topWall = y == 0 ? new() : cells[x, y - 1].bottomWall,
                    bottomWall = new(),
