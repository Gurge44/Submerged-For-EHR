...
public sealed class CustomKillAnimationPlayer : MonoBehaviour
{
    ...
    private readonly SCG.Dictionary<int, KillAnimFrame> _frames = new();
    ...
    public void UpdateVisuals(float sampleTime, Vector2 characterOffset, int animation)
    {
        ...
        AnimationClip bodyClip =
            animCosmetics.bodySprites._items[0].BodySprite.GetComponent<SpriteAnim>().m_defaultAnim;
    ...
    public void UpdateVisuals(float sampleTime, Vector2 characterOffset, AnimationClip bodyAnimClip,
        AnimationClip skinAnimClip)
    {
        AnimationClip.SampleAnimation(_bodyAnim.gameObject, bodyAnimClip, sampleTime, WrapMode.Default);
        if (skinAnimClip)
            AnimationClip.SampleAnimation(_skinAnim.gameObject, skinAnimClip, sampleTime, WrapMode.Default);
